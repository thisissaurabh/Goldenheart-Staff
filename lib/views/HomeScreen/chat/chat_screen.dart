...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    //   // backpress();
    //   debugPrint("Check didPop: $didPop");
    //   if (!didPop) {
    //     backpress();
    //   }
    // },
                                  ...
                                  if (snapshot.connectionState.name ==
                                    ...
                                    if (snapshot.hasError) {
                                      ...
                                      if (messageList.isNotEmpty &&
                                          snapshot.hasData) {
                                        debugPrint('Check First Message');
                                        debugPrint(
                                      ...
                                      print(
                                          ' Message Lenght ==> ${messageList.length}');
                                      ...
                                      child: Container(
                                        ...
                                        child: TextFormField(
                                          keyboardType: TextInputType
                                              .name, // Ensures only alphabetic keyboard
                                          textCapitalization:
                                            ...
                                            if (RegExp(r'\d').hasMatch(value)) {
                                              ...
                                              if (!hasShownSnackbar) {
                                                ...
                                                Future.delayed(
                                                    ...
                                                    () {
                                                  hasShownSnackbar =
                                                      false; // Reset the flag after some time
                                                });
                                              }
                                            } else {
                                              hasShownSnackbar =
                                                  false; // Reset flag if input becomes valid
                                            }
                                          },
  ...
  void backpress() async {
    print('inside it appbar click');
    callController.newIsStartTimer = false;
    callController.update();
    await walletController.getAmountList();

    if (widget.flagId == 1) {
      print('check ended chat');
      chatController.sendMessage(
          '${global.user.name} -> Chat Closed', widget.customerId, true);
