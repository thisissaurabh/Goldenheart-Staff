...
//Update astrologer
  ...
  /// update Astrologer new

  CurrentUser updateUpdateProfile = CurrentUser();
  Future updateAstrologerProfile(int id) async {
    try {
      await global.checkBody().then(
        (networkResult) async {
          if (networkResult) {
            print('Print roleId ======${global.user.roleId}');
            print('Print name ======${global.user.name}');
            print('Print email ======${global.user.email}');
            print('Print contactNo ======${global.user.contactNo}');
            print('Print imagePath ======${global.user.imagePath}');
            print('Print gender ======${global.user.gender}');
            print('Print birthDate ======${global.user.birthDate}');
            print('Print primarySkill ======${global.user.primarySkill}');
            print(
                'Print astrologerCategory ======${global.user.astrologerCategory}');
            print('Print allSkill ======${global.user.allSkillId}');
            print('Print ${global.user.languageId!}');
            print('Print charges ======${global.user.charges}');
            print('Print videoCallRate ======${global.user.videoCallRate}');
            print('Print reportRate ======${global.user.reportRate}');
            print('Print expirenceInYear ======${global.user.expirenceInYear}');
            print(
                'Print dailyContributionHours ======${global.user.dailyContributionHours}');
            print(
                'Print hearAboutAstroGuru ======${global.user.hearAboutAstroGuru}');
            print(
                'Print isWorkingOnAnotherPlatform ======${global.user.isWorkingOnAnotherPlatform}');
            print(
                'Print otherPlatformName ======${global.user.otherPlatformName}');
            print(
                'Print otherPlatformMonthlyEarning ======${global.user.otherPlatformMonthlyEarning}');
            print('Print onboardYou ======${global.user.onboardYou}');
            print(
                'Print suitableInterviewTime ======${global.user.suitableInterviewTime}');
            print('Print currentCity ======${global.user.currentCity}');
            print(
                'Print mainSourceOfBusiness ======${global.user.mainSourceOfBusiness}');
            print(
                'Print highestQualification ======${global.user.highestQualification}');
            print('Print degreeDiploma ======${global.user.degreeDiploma}');
            print(
                'Print collegeSchoolUniversity ======${global.user.collegeSchoolUniversity}');
            print('Print learnAstrology ======${global.user.learnAstrology}');
            print(
                'Print instagramProfileLink ======${global.user.instagramProfileLink}');
            print(
                'Print facebookProfileLink ======${global.user.facebookProfileLink}');
            print(
                'Print linkedInProfileLink ======${global.user.linkedInProfileLink}');
            print(
                'Print youtubeProfileLink ======${global.user.youtubeProfileLink}');
            print(
                'Print webSiteProfileLink ======${global.user.webSiteProfileLink}');
            print('Print isAnyBodyRefer ======${global.user.isAnyBodyRefer}');
            print(
                'Print expectedMinimumEarning ======${global.user.expectedMinimumEarning}');
            print(
                'Print expectedMaximumEarning ======${global.user.expectedMaximumEarning}');
            print('Print longBio ======${global.user.longBio}');
            print(
                'Print foreignCountryCount ======${global.user.foreignCountryCount}');
            print(
                'Print currentlyWorkingJob ======${global.user.currentlyWorkingJob}');
            print(
                'Print goodQualityOfAstrologer ======${global.user.goodQualityOfAstrologer}');
            print(
                'Print biggestChallengeFaced ======${global.user.biggestChallengeFaced}');
            print(
                'Print repeatedQuestion ======${global.user.repeatedQuestion}');

            int a = global.user.userId!;
            updateUpdateProfile.id = id;
            updateUpdateProfile.userId = a;
            updateUpdateProfile.roleId = 2;
            updateUpdateProfile.name = global.user.name;
            updateUpdateProfile.email = cEmail.text;
            updateUpdateProfile.contactNo = cMobileNumber.text;
            if (profile != null && profile != '') {
              updateUpdateProfile.imagePath = profile;
            }
            updateUpdateProfile.gender = selectedGender;
            updateUpdateProfile.birthDate = selectedDate;
            updateUpdateProfile.primarySkill = cPrimarySkill.text;
            updateUpdateProfile.astrologerCategory = cSelectCategory.text;
            updateUpdateProfile.allSkill = cAllSkill.text;
            updateUpdateProfile.languageKnown = cLanguage.text;
            updateUpdateProfile.charges = int.parse(cCharges.text);
            updateUpdateProfile.videoCallRate = int.parse(cVideoCharges.text);
            updateUpdateProfile.reportRate = int.parse(cReportCharges.text);
            updateUpdateProfile.expirenceInYear = int.parse(cExpirence.text);
            updateUpdateProfile.dailyContributionHours =
                int.parse(cContributionHours.text);
            updateUpdateProfile.hearAboutAstroGuru = cHearAboutAstroGuru.text;
            updateUpdateProfile.isWorkingOnAnotherPlatform = anyOnlinePlatform;
            updateUpdateProfile.otherPlatformName = cNameOfPlatform.text;
            updateUpdateProfile.otherPlatformMonthlyEarning =
                cMonthlyEarning.text;
            updateUpdateProfile.onboardYou = cOnBoardYou.text;
            updateUpdateProfile.suitableInterviewTime = cTimeForInterview.text;
            updateUpdateProfile.currentCity = cLiveCity.text;
            updateUpdateProfile.mainSourceOfBusiness = selectedSourceOfBusiness;
            updateUpdateProfile.highestQualification =
                selectedHighestQualification;
            updateUpdateProfile.degreeDiploma = selectedDegreeDiploma;
            updateUpdateProfile.collegeSchoolUniversity =
                cCollegeSchoolUniversity.text;
            updateUpdateProfile.learnAstrology = cLearnAstrology.text;
            updateUpdateProfile.instagramProfileLink = cInsta.text;
            updateUpdateProfile.facebookProfileLink = cFacebook.text;
            updateUpdateProfile.linkedInProfileLink = cLinkedIn.text;
            updateUpdateProfile.youtubeProfileLink = cYoutube.text;
            updateUpdateProfile.webSiteProfileLink = cWebSite.text;
            updateUpdateProfile.isAnyBodyRefer = referPerson;
            updateUpdateProfile.referedPersonName = cNameOfReferPerson.text;
            updateUpdateProfile.expectedMinimumEarning =
                int.parse(cExptectedMinimumEarning.text);
            updateUpdateProfile.expectedMaximumEarning =
                int.parse(cExpectedMaximumEarning.text);
            updateUpdateProfile.longBio = cLongBio.text;
            updateUpdateProfile.foreignCountryCount =
                selectedForeignCountryCount;
            updateUpdateProfile.currentlyWorkingJob =
                selectedCurrentlyWorkingJob;
            updateUpdateProfile.goodQualityOfAstrologer = cGoodQuality.text;
            updateUpdateProfile.biggestChallengeFaced = cBiggestChallenge.text;
            updateUpdateProfile.repeatedQuestion = cRepeatedQuestion.text;
            updateUpdateProfile.token = global.user.token;
            updateUpdateProfile.tokenType = global.user.tokenType;
            updateUpdateProfile.week = [];

            for (var i = 0; i < week!.length; i++) {
              if (week![i].timeAvailabilityList!.isNotEmpty) {
                updateUpdateProfile.week!.add(Week(
                    day: week![i].day,
                    timeAvailabilityList: week![i].timeAvailabilityList));
              }
            }
            ...
            //Astrologer category
            updateUpdateProfile.astrologerCategoryId = [];
            if (astroId.isEmpty) {
              for (var i = 0;
                  i < global.user.astrologerCategoryId!.length;
                  i++) {
              ...
              for (var i = 0; i < astroId.length; i++) {
                updateUpdateProfile.astrologerCategoryId!.add(astroId[i]);
              }
            }

            //Primary skill
            updateUpdateProfile.primarySkillId = [];
            if (primaryId.isEmpty) {
              for (var i = 0; i < global.user.primarySkillId!.length; i++) {
                updateUpdateProfile.primarySkillId!
                    .add(global.user.primarySkillId![i]);
              }
            } else {
              for (var i = 0; i < primaryId.length; i++) {
                updateUpdateProfile.primarySkillId!.add(primaryId[i]);
              }
            }

            //All skill
            updateUpdateProfile.allSkillId = [];
            if (allId.isEmpty) {
              for (var i = 0; i < global.user.allSkillId!.length; i++) {
                updateUpdateProfile.allSkillId!.add(global.user.allSkillId![i]);
              }
            } else {
              for (var i = 0; i < allId.length; i++) {
                updateUpdateProfile.allSkillId!.add(allId[i]);
              }
            }

            //language known
            updateUpdateProfile.languageId = [];
            if (lId.isEmpty) {
              for (var i = 0; i < global.user.languageId!.length; i++) {
                updateUpdateProfile.languageId!.add(global.user.languageId![i]);
              }
            } else {
              for (var i = 0; i < lId.length; i++) {
                updateUpdateProfile.languageId!.add(lId[i]);
              }
            }

            await apiHelper.astrologerUpdate(updateUpdateProfile).then(
              (apiResult) async {
                if (apiResult.status == '200') {
                  global.user = updateUpdateProfile;
                  signupController.astrologerList.clear();
                  await signupController.astrologerProfileById(false);
                  update();
                  await global.sp!.setString(
  ...
  // Future<File?> pickImageService(ImageSource source) async {
  //   final pickedFile = await ImagePicker().pickImage(source: source);
  //   return pickedFile != null ? File(pickedFile.path) : null;
  // }
  Future<void> updateProfileApi(String name, String? birthDate, String userId,
      String id, String profileImage,
      {bool isImageIncluded = false}) async {
    global.showOnlyLoaderDialog();
    update();

    var headers = await global.getApiHeaders(true);
    var url = Uri.parse(editProfile);
    var request = http.MultipartRequest('POST', url);

    // Log API URL
    print('API URL: $editProfile');
    ...
    if (isImageIncluded) {
      print('Image is included in the request.');
      if (profileImage.startsWith('/')) {
        print('Image is a file path: $profileImage');
        if (File(profileImage).existsSync()) {
          print('File exists, adding to request...');
          request.files.add(
              await http.MultipartFile.fromPath('profileImage', profileImage));
        } else {
          print(
              '❌ Error: File does not exist at the provided path: $profileImage');
          global.hideLoader();
          return;
        }
      } else {
        print('Image is a Base64 string.');
        try {
          final imageBytes = base64Decode(profileImage);
          print(
              'Decoded Base64 image successfully. Size: ${imageBytes.length} bytes');
          request.files.add(
      ...
      print('No image included in the request.');
    }
    ...
    try {
      ...
      Get.find<SplashController>().performApiCalls();
      global.hideLoader();
      Get.back();
      update();
    }
